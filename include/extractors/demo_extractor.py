...
class MyPythonExtractor(BaseExtractor):
    ...
    def extract(self) -> TaskMetadata:
        ...
        try:
            ...
            all_args = {
                ("arg" + str(idx + 1)): arg
                for idx, arg in enumerate(self.operator.op_args)
            }
            all_args.update(self.operator.op_kwargs)

            input = InputDataset(
                name=f"{task_name}_python_input", namespace="python://args"
            )
            ...
            datasource = DataSourceDatasetFacet(
                name=f"{task_name}_python_input", uri="python://args"
            )
    ...
    def extract_on_complete(self, task_instance) -> Optional[TaskMetadata]:
        ...
        if self.operator.do_xcom_push is True:
            ...
            ti = self.operator.get_task_instances(
            ...
            if len(ti) > 0:
                ...
                return_value = task_instance.xcom_pull(
                    task_ids=task_id, dag_id=dag_id, key="return_value"
                )
                if return_value is not None:
                    log.debug(f"return value received: {return_value}")
                    # populate output variable to output
                    output = OutputDataset(
                        name=f"{task_name}_python_output", namespace="python://args"
                    )
                    fields = []
                    fields.append(
                        SchemaField(
                    ...
                    datasource = DataSourceDatasetFacet(
                        name=f"{task_name}_python_output", uri="python://args"
                    )
